...
### Fixed
- Fixed `unhandledRejection` error caused by an orphaned promise in `DeduplicationManager` when a request was rejected.
- Disabled `logRequests` and `logResponses` by default to prevent unintentional exposure of sensitive data in debug mode (Security Fix SBP-005).
